...
long long mention_size, feature_size, type_size;
long long mf_size, my_size;
...
void LearnVocabFromTrainFile() {
    ...
    while (fgets(str, sizeof(str), fin))
    {
        ...
        for (k = 0; k != len; k++) if (str[k] == '\t')
            break;
        pst = atoi(str + k + 1);
        if (pst >= mention_size) mention_size = pst + 1;
    }
    fclose(fin);
    ...
    vocab_size = mention_size + feature_size + type_size;
    ...
    while (fgets(str, sizeof(str), fin))
    {
        ...
        str[k] = 0;
        gid = atoi(str + k + 1);
        pst = gid + mention_size + feature_size;
...
void ReadNet()
{
    ...
    while (1)
    {
        ...
        if (my_size % 100000 == 0)
        {
            printf("%lldK%c", my_size / 1000, 13);
            fflush(stdout);
        }
        
        u += 0;
        v += mention_size + feature_size;
        neighbor[u].push_back(v);
        neighbor[v].push_back(u);
        vocab[u].cn += wei;
        vocab[v].cn += wei;
...
void TrainModel() {
    ...
    sprintf(file_name, "%semb_dw_mention.txt", output_path);
    fo = fopen(file_name, "wb");
    if (binary == 1)
    {
        fprintf(fo, "%lld %lld\n", mention_size, layer1_size);
        for (int a = 0; a != mention_size; a++)
        {
    ...
    sprintf(file_name, "%semb_dw_feature.txt", output_path);
    fo = fopen(file_name, "wb");
    if (binary == 1)
    {
        fprintf(fo, "%lld %lld\n", feature_size, layer1_size);
        for (int a = mention_size; a != mention_size + feature_size; a++)
        {
            fprintf(fo, "%s ", vocab[a].word);
            for (int b = 0; b != layer1_size; b++) fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fo);
            fprintf(fo, "\n");
    ...
    {
        fprintf(fo, "%lld %lld\n", feature_size, layer1_size);
        for (int k = mention_size; k != mention_size + feature_size; k++)
        {
            fprintf(fo, "%s\t%d\t", vocab[k].word, vocab[k].gid);
            for (int c = 0; c != layer1_size; c++) fprintf(fo, "%lf ", syn0[k * layer1_size + c]);
            fprintf(fo, "\n");
    ...
    sprintf(file_name, "%semb_dw_type.txt", output_path);
    fo = fopen(file_name, "wb");
    if (binary == 1)
    {
        fprintf(fo, "%lld %lld\n", type_size, layer1_size);
        for (int a = mention_size + feature_size; a != mention_size +feature_size + type_size; a++)
        {
            fprintf(fo, "%s ", vocab[a].word);
            for (int b = 0; b != layer1_size; b++) fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fo);
            fprintf(fo, "\n");
    ...
    {
        fprintf(fo, "%lld %lld\n", type_size, layer1_size);
        for (int k = mention_size + feature_size; k != mention_size + feature_size + type_size; k++)
        {
            fprintf(fo, "%s\t%d\t", vocab[k].word, vocab[k].gid);
            for (int c = 0; c != layer1_size; c++) fprintf(fo, "%lf ", syn0[k * layer1_size + c]);
            fprintf(fo, "\n");
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-iters", argc, argv)) > 0) walks_per_vertex = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-length", argc, argv)) > 0) walk_length = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-window", argc, argv)) > 0) window = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-alpha", argc, argv)) > 0) alpha = atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-threads", argc, argv)) > 0) num_threads = atoi(argv[i + 1]);
    sprintf(file_path, "data/intermediate/%s/rm/", data);
    sprintf(output_path, "data/results/%s/rm/", data);
