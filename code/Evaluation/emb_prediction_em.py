...
class Predicter_useFeatureEmb:
    ...
    def predict_types_for_em_maximum(self, mention_id, _threshold, candidate):
        _type_size = self._embs_type._node_size
        _emb_mention = self.get_mention_embedding(mention_id)
        ...
        for i in candidate:
            ...
            if max_score < score:
                max_index = i
                max_score = score
        labels.append((max_index, max_score))

        # Add parent of max_index if any
        temp = max_index
        while temp in parent_mapping:
            _emb_type = self._embs_type.get_embedding(parent_mapping[temp])
            score = sim_func(_emb_mention, _emb_type, self._sim_func)
            labels.append((parent_mapping[temp], score))
        ...
        while temp != -1:
            ...
            for child in candidate:
                ...
                if child not in parent_mapping:
                    continue
            ...
            if max_sub_index != -1 and max_sub_score > _threshold:
                labels.append((max_sub_index, max_sub_score))
            temp = max_sub_index
        return labels
...
def predict(indir, outdir, _method, _sim_func, _threshold, output, none_label_index):
    ...
    with open(os.path.join(indir + '/mention_feature_test.txt')) as f,\
        ...
        types = None
        """
        if os.path.isfile(os.path.join(indir + '/em_type_hierarchy.txt')):
            f = open(os.path.join(indir + '/em_type_hierarchy.txt'), 'r')
            types = set()
            for t in f.readlines():
                types.add(t.strip('\r\n'))
        """
        all_candidates = load_all_candidates(os.path.join(indir + '/type.txt'), mentions_ids, types)
        cnt = 0
        pos_cnt = 0
        mentions_tested = set()
        labels = []
        ...
        for line in f:
            ...
            if mention_id not in mentions_tested:
                ...
                pairs = predicter.predict_types_for_em_maximum(mention_id, _threshold, all_candidates[mention_id])
        ...
        f.close()
        g.close()
    # print pos_cnt, '/', cnt, 'are detected as mentios'
...
def load_all_candidates(filename, indexes, types):
    """
    Load data as a dict of list.
    e.g.{0:[0,1,2],1:[1,2]}
    """
    ...
    with open(filename) as f:
        for line in f:
            seg = line.strip('\r\n').split('\t')
            if len(seg) == 3:
                if (types is None) or (seg[0] in types):
                    tid = int(seg[1])
